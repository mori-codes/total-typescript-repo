...
const makeFormValidatorFactory =
  <TValidators extends Record<string, (value: string) => void | string>>(
    validators: TValidators
  ) =>
  <TFields extends string>(
    config: Record<TFields, Array<keyof TValidators>>
  ) => {
    return (values: Record<TFields, string>) => {
      const errors = {} as Record<TFields, undefined | string>
